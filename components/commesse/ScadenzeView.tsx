...
import {
  Plus, Pencil, Trash2, Camera, Check,
} from 'lucide-react'
...
export default function ScadenzeView({ gruppoId, gruppoNome, scadenze }: Props) {
  ...
  // URL firmati delle foto + upload in corso
        ...
        {MESI.map((nomeMese, i) => {
          ...
          return (
            <div key={mese} className="rounded-md border bg-white overflow-hidden">
              <div className="flex items-center justify-between px-4 py-2.5 bg-gray-50/70 border-b">
                ...
                <Button
                  ...
                  onClick={() => setDialog({ scadenza: null, defaultData })}
                >
              ...
              {righe.length > 0 && (
                <div className="divide-y">
                  {righe.map((s) => (
                    <div key={s.id} className={`flex items-center gap-3 px-4 py-2.5 ${s.pagato ? 'bg-emerald-50/40' : ''}`}>
                      {/* Pagato toggle */}
                      <button
                        type="button"
