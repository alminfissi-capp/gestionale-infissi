...
- list_clienti: elenca i clienti dell'organizzazione
- get_cliente: recupera i dettagli di un cliente tramite id
- list_preventivi: elenca i preventivi, con filtro opzionale per stato
- get_preventivo: recupera i dettagli di un preventivo tramite id
- create_cliente: raccoglie i dati per creare un nuovo cliente (richiede conferma umana)
- navigate_to: naviga verso una pagina dell'applicazione (richiede conferma umana)`,
    ...
    tools: {
      ...
      list_preventivi: tool({
        description: 'Elenca i preventivi dell\'organizzazione, con join ai clienti. Filtro opzionale per stato.',
        parameters: z.object({
          stato: z
          ...
          limit: z
            ...
            .optional()
            .default(20)
            .describe('Numero massimo di risultati (default 20, max 100)'),
        }),
      ...
      create_cliente: tool({
        description:
          'Crea un nuovo cliente. Richiede conferma prima dell\'esecuzione. Raccoglie i dati necessari.',
        parameters: z.object({
      ...
      navigate_to: tool({
        description: 'Naviga verso una pagina dell\'applicazione. Richiede conferma prima dell\'esecuzione.',
        parameters: z.object({
          path: z.string().describe('Percorso della pagina (es. /clienti, /preventivi)'),
          label: z.string().describe('Nome leggibile della destinazione (es. "lista clienti")'),
        }),
      }),
    },
  })
