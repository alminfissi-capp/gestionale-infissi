-- Finiture per categorie alluminio/ferro (magazzino)
CREATE TABLE finiture_magazzino (
...
ALTER TABLE finiture_magazzino ENABLE ROW LEVEL SECURITY;
CREATE POLICY "finiture_mag_select" ON finiture_magazzino FOR SELECT USING (organization_id = get_user_organization_id());
CREATE POLICY "finiture_mag_insert" ON finiture_magazzino FOR INSERT WITH CHECK (organization_id = get_user_organization_id());
CREATE POLICY "finiture_mag_update" ON finiture_magazzino FOR UPDATE USING (organization_id = get_user_organization_id());
CREATE POLICY "finiture_mag_delete" ON finiture_magazzino FOR DELETE USING (organization_id = get_user_organization_id());
...
ALTER TABLE movimenti_magazzino
  ADD COLUMN finitura_id UUID REFERENCES finiture_magazzino(id) ON DELETE SET NULL,
